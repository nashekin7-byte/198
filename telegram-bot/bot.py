...
async def show_appointment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    appointment_text += f"*📱 Способы связи:*\n\n"
    appointment_text += f"• Telegram: {CLINIC_INFO['telegram_channel']}\n"
    appointment_text += f"• Телефон: {CLINIC_INFO['phone']}\n"
    appointment_text += f"• Сайт: {CLINIC_INFO['website']}\n\n"
    
    appointment_text += f"*⏰ Режим работы:*\n"
    appointment_text += f"{CLINIC_INFO['working_hours']}\n\n"
    
    appointment_text += f"*📍 Адрес:*\n"
    appointment_text += f"{CLINIC_INFO['address']}\n\n"
    
    appointment_text += f"*ℹ️ О первом визите:*\n"
    appointment_text += f"Длительность: *30\-40 минут*\n"
    appointment_text += f"Включает: осмотр + консультация + расчет стоимости лечения\n\n"
    
    appointment_text += f"Запишитесь сегодня и получите полную консультацию от наших специалистов\!"
    ...
    try:
        if isinstance(update, Update) and update.effective_message:
            await update.effective_message.reply_text(
                "⚠️ Произошла ошибка\. Пожалуйста, попробуйте ещё раз или свяжитесь с нами напрямую\:\n\n"
                f"📱 {CLINIC_INFO['telegram_channel']}\n"
                f"📞 {CLINIC_INFO['phone']}"
            )
    except Exception as e:
